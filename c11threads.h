 ...
 * scspell-id: 9aea287a-0f92-11ee-a84a-80ee73e9b8e7
 *
 * ---------------------------------------------------------------------------
 *
 * c11threads.h
 ...
 * implementation of C11 threads), then just define
 * C11THREADS_PTHREAD_WIN32 before including this header file.
 */
...
/*
 * If your compiler does not support the inline keyword, or
 * supports it with some different variation of prefix or
 * suffix underscores, you can define C11THREADS_INLINE before
...
/*
 * MSVC has inline from VS 2015 but supports
 * __inline in older versions
 */
...
/*
 * For every other case, just gamble on having __inline__,
...
/*
 * C11 threads over POSIX threads as thin
...
#  endif /* ifdef C11THREADS_NO_TIMED_MUTEX */
...
#  ifdef C11THREADS_NO_TIMED_MUTEX
   ...
   * in a loop and waiting for a bit
   */
...
/*
 * Win32: Threads not created with thrd_create()
 * need to call this to clean up TSS.
 */
...
/*
 * Win32: Register current Win32 thread in
 * c11threads to allow for proper thrd_join().
 */

int c11threads_win32_thrd_self_register(void);

/*
 * Win32: Register Win32 thread by ID in
 * c11threads to allow for proper thrd_join().
 */
