 ...
 * scspell-id: 8ce1e25e-0f92-11ee-921c-80ee73e9b8e7
 *
 * -------------------------------------------------------------------------
 *
 * c11threads_win32.c
 *
 * C11Threads is forked from c11threads; the original version
 * is available from https://github.com/jtsiomb/c11threads
 * Based on upstream commit ec95e1aa82079aefe109d18e5069b79711692064
 *
 * ---------------------------------------------------------------------------
 *
 * Original authors:
 ...
 * of the code somewhere, but whatever.
 *
 * ---------------------------------------------------------------------------
 */
...
/*
 * Map debug malloc and free functions for debug builds.
...
#  undef _WIN32_WINNT_VISTA
# endif /* ifdef _WIN32_WINNT_VISTA */
# define _WIN32_WINNT_VISTA  0x0600

# ifdef THREAD_QUERY_LIMITED_INFORMATION
#  undef THREAD_QUERY_LIMITED_INFORMATION
# endif /* ifdef THREAD_QUERY_LIMITED_INFORMATION */
...
typedef void ( __stdcall *_c11threads_win32_InitializeConditionVariable_t )
(
  void *
);

typedef void ( __stdcall *_c11threads_win32_WakeConditionVariable_t )
(
  void *
);

typedef void ( __stdcall *_c11threads_win32_WakeAllConditionVariable_t )
(
  void *
);

typedef int ( __stdcall *_c11threads_win32_SleepConditionVariableCS_t )
(
  void *,
  PCRITICAL_SECTION,
  unsigned long
...
typedef int ( __stdcall *_c11threads_win32_InitOnceExecuteOnce_t )
(
  void *,
  const void *,
  void *,
...
static unsigned short
  _c11threads_win32_winver;

static _c11threads_win32_InitializeConditionVariable_t
  _c11threads_win32_InitializeConditionVariable;

static _c11threads_win32_WakeConditionVariable_t
  _c11threads_win32_WakeConditionVariable;

static _c11threads_win32_WakeAllConditionVariable_t
  _c11threads_win32_WakeAllConditionVariable;

static _c11threads_win32_SleepConditionVariableCS_t
  _c11threads_win32_SleepConditionVariableCS;

static _c11threads_win32_InitOnceExecuteOnce_t
  _c11threads_win32_InitOnceExecuteOnce;

static CRITICAL_SECTION
  _c11threads_win32_thrd_list_critical_section;

static struct _c11threads_win32_thrd_entry_t
  * _c11threads_win32_thrd_list_head = NULL;

static CRITICAL_SECTION
  _c11threads_win32_tss_dtor_list_critical_section;

static struct _c11threads_win32_tss_dtor_entry_t
...
static void
_c11threads_win32_init(void)
{
  unsigned short os_version;

  os_version                = (unsigned short) GetVersion();
  ...
  if (_c11threads_win32_winver >= _WIN32_WINNT_VISTA)
    {
      ...
      _c11threads_win32_InitializeConditionVariable
        = (_c11threads_win32_InitializeConditionVariable_t)GetProcAddress(
            kernel32,
      ...
      _c11threads_win32_WakeConditionVariable
        = (_c11threads_win32_WakeConditionVariable_t)GetProcAddress(
            kernel32,
      ...
      _c11threads_win32_WakeAllConditionVariable
        = (_c11threads_win32_WakeAllConditionVariable_t)GetProcAddress(
            kernel32,
      ...
      _c11threads_win32_SleepConditionVariableCS
        = (_c11threads_win32_SleepConditionVariableCS_t)GetProcAddress(
            kernel32,
      ...
      _c11threads_win32_InitOnceExecuteOnce
        = (_c11threads_win32_InitOnceExecuteOnce_t)GetProcAddress(
            kernel32,
            "InitOnceExecuteOnce");
...
static void
_c11threads_win32_ensure_initialized(void)
{
...
static int
_c11threads_win32_util_is_timespec32_valid(
  const struct _c11threads_win32_timespec32_t *ts)
{
  return ts->tv_sec >= 0 && ts->tv_nsec >= 0 && ts->tv_nsec <= 999999999;
}

static int
_c11threads_win32_util_is_timespec64_valid(
  const struct _c11threads_win32_timespec64_t *ts)
{
  return ts->tv_sec >= 0 && ts->tv_nsec >= 0 && ts->tv_nsec <= 999999999;
}
...
static __int64
_c11threads_win32_util_timespec32_to_file_time(
  const struct _c11threads_win32_timespec32_t *ts)
{
  ...
  nsec_res = (unsigned long)ts->tv_nsec / 100UL
             + !!((unsigned long)ts->tv_nsec % 100UL );
...
static __int64
_c11threads_win32_util_timespec64_to_file_time(
  const struct _c11threads_win32_timespec64_t *ts, size_t *periods)
{
  unsigned __int64  sec_res;
  unsigned __int64  nsec_res;
  unsigned __int64  res;

  *periods  = (unsigned long)((unsigned __int64)ts->tv_sec
                / (unsigned __int64)922337203685);
...
static int
_c11threads_win32_util_timespec32_to_milliseconds(
  const struct _c11threads_win32_timespec32_t *ts, unsigned long *ms)
{
  ...
  nsec_res  = (unsigned long)ts->tv_nsec / 1000000UL
...
static int
_c11threads_win32_util_timespec64_to_milliseconds(
  const struct _c11threads_win32_timespec64_t *ts, unsigned long *ms)
{
  unsigned long  sec_res;
  unsigned long  nsec_res;
...
_c11threads_win32_util_timepoint_to_millisecond_timespan32(
  const struct _c11threads_win32_timespec32_t *current_time,
  const struct _c11threads_win32_timespec32_t *end_time, int *clamped)
{
...
static unsigned long
_c11threads_win32_util_timepoint_to_millisecond_timespan64(
  const struct _c11threads_win32_timespec64_t *current_time,
  const struct _c11threads_win32_timespec64_t *end_time, int *clamped)
{
  unsigned long                          wait_time;
                                 ...
                                 int base)
{
  ...
  /*
   * Also, subtract difference between FILETIME and UNIX time epoch.
   * It's 369 years, by the way.
   */

  ts->tv_sec   = (long)( li.QuadPart / (unsigned __int64)10000000
                           - (unsigned __int64)11644473600 );
...
int
_c11threads_win32_timespec64_get(struct _c11threads_win32_timespec64_t *ts,
                                 int base)
{
  FILETIME        file_time;
  ULARGE_INTEGER  li;
  ...
  ts->tv_sec   = li.QuadPart / (unsigned __int64)10000000
                   - (unsigned __int64)11644473600;
...
# endif /* if defined( C11THREADS_NO_TIMESPEC_GET ) || !defined( _MSC_VER ) */
...
static int
_c11threads_win32_thrd_register(thrd_t thrd, HANDLE h)
{
...
static void *
_c11threads_win32_thrd_pop_entry(thrd_t thrd)
{
  void *                                  h;
...
static void
_c11threads_win32_thrd_run_tss_dtors(void)
{
  int                                         ran_dtor;
  size_t                                      i;
...
int
c11threads_win32_thrd_self_register(void)
{
...
int
c11threads_win32_thrd_register(unsigned long win32_thread_id)
{

  /*
   * Temporary hack to make this build on MSVC6.
   */
...
# endif /* ifdef _PROCESSTHREADSAPI_H_ */
...
thrd_create(thrd_t *thr, thrd_start_t func, void *arg)
{
  ...
  h = CreateThread(
    NULL,
    0,
    (PTHREAD_START_ROUTINE)_c11threads_win32_thrd_start_thunk,
    thread_start_params,
    0,
    thr);
  ...
  if (h)
    {
      ...
      if (WaitForSingleObject(h, INFINITE) == WAIT_OBJECT_0
          && ( !res || GetExitCodeThread(h, (unsigned long *)res)))
        {
          ret = thrd_success;
        }
...
static int
_c11threads_win32_sleep_common(__int64 file_time_in)
{
  void *         timer;
  unsigned long  error;
...
int
_c11threads_win32_thrd_sleep32(
...
{
  return TryEnterCriticalSection((PCRITICAL_SECTION)mtx) ? thrd_success
                                                         : thrd_busy;
}

int
_c11threads_win32_mtx_timedlock32(
  ...
  while (!success)
    {
      ...
      if (ts_current.tv_sec > ts->tv_sec
          || ( ts_current.tv_sec == ts->tv_sec
               && ts_current.tv_nsec >= ts->tv_nsec ))
        {
...
int
_c11threads_win32_mtx_timedlock64(
      ...
      if (success)
        {
          ...
          if (cnd->wait_count)
            {
              success = ReleaseSemaphore(cnd->signal_sema, 1, NULL)
...
static int
_c11threads_win32_cnd_wait_common(cnd_t *cond, mtx_t *mtx,
                                  unsigned long wait_time, int clamped)
{
  _c11threads_win32_ensure_initialized();
  ...
  if (_c11threads_win32_winver >= _WIN32_WINNT_VISTA)
  ...
  else
    {
      ...
      wait_status = WaitForMultipleObjects(
        2,
        &cnd->signal_sema /* and cnd->broadcast_event */,
        0,
        wait_time);
...
int
_c11threads_win32_cnd_timedwait32(
  cnd_t *cond, mtx_t *mtx, const struct _c11threads_win32_timespec32_t *ts)
{
  ...
  wait_time = _c11threads_win32_util_timepoint_to_millisecond_timespan32(
    &current_time,
    ts,
    &clamped);
...
int
_c11threads_win32_cnd_timedwait64(
  cnd_t *cond, mtx_t *mtx, const struct _c11threads_win32_timespec64_t *ts)
{
  ...
  wait_time = _c11threads_win32_util_timepoint_to_millisecond_timespan64(
    &current_time,
    ts,
    &clamped);
...
static int
_c11threads_win32_tss_register(tss_t key, tss_dtor_t dtor)
{
  struct _c11threads_win32_tss_dtor_entry_t *tss_dtor_entry;
...
static void
_c11threads_win32_tss_deregister(tss_t key)
{
  struct _c11threads_win32_tss_dtor_entry_t * prev;
  struct _c11threads_win32_tss_dtor_entry_t * curr;
  struct _c11threads_win32_tss_dtor_entry_t * next;
...
static int __stdcall _c11threads_win32_call_once_thunk(void *  init_once,
                                                       void (* func ) (void),
                                                       void ** context)
{
  (void)init_once;
